...
one.statistic <- function(k, benefits.cumsum, n, overall.mean, maxonly=F){
  result  <- (benefits.cumsum[((2+k):n)-1] - benefits.cumsum[(2:(n-k))-1]) - k*overall.mean
  best.ix <- which.max(result)
  return(
    c(1+best.ix, k+best.ix, max(result), k)
  )
}


compute.all.statistics <- function(n, benefit.cs, start, end, maxonly=F){
  result <- do.call(rbind,  lapply(start:(end-1), function(k) one.statistic(k, benefit.cs, n, benefit.cs[n]/n)))
  ...
  return(
    list(start.index  = best[1],
         ...
         mean.inside  = (benefit.cs[best[2]] - benefit.cs[best[1]-1])/best[4],
         mean.outside = (benefit.cs[n] - benefit.cs[best[2]] + benefit.cs[best[1]-1])/(n - best[4]),
         statistic    = best[3]
    )
  )
}

find.sweetspot <- function(observed.benefit, ntrials.significance=1000, ntrials.bias=1000,
                           min.size.fraction = 1/20, max.size.fraction = 1, ncores=NULL){
  ...
  best.window      <- compute.all.statistics(n, cumsum(observed.benefit), search.start, search.end)

  # Compute the p-value
  pvalue <- foreach(i=1:ntrials.significance, .combine = sum, .inorder=F, .multicombine=T, .maxcombine=ntrials.significance) %dopar% {
    compute.all.statistics(n, cumsum(sample(observed.benefit, replace=F)), search.start, search.end, maxonly=T) >= best.window$statistic
  }
  ...
  debias.boot <- foreach(i=1:ntrials.bias, .combine = rbind, .inorder=F, .multicombine=T, .maxcombine=ntrials.bias) %dopar% {
    compute.all.statistics(n,
                           cumsum(c(sample(values.outside, best.window$start.index - 1, replace=T),
                           ...
                           search.start, search.end, maxonly=F)
  }
